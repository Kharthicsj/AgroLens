...
const Signup = () => {
  ...
  const handleSignup = async () => {
    ...
    if (passwordErrors.length > 0) {
      Alert.alert(
        t('weakPassword'),
        t('passwordMustContain') + ' ' + passwordErrors.join(', ') + '.'
      );
      return;
    }
    ...
    try {
      ...
      setLoading(false);
      const errorMessage = error.message || t('accountCreationFailed');
          ...
          >
            ...
            >
              <View style={SignupStyles.formContainer}>
                <Text style={SignupStyles.title}>{t('createAccount')}</Text>
                <Text style={SignupStyles.subtitle}>{t('joinUsSubtitle')}</Text>

                <View style={SignupStyles.inputContainer}>
                  <Text style={SignupStyles.label}>{t('fullName')}</Text>
                  <TextInput
                ...
                <View style={SignupStyles.inputContainer}>
                  <Text style={SignupStyles.label}>{t('confirmPassword')}</Text>
                  <PasswordInput
                    style={SignupStyles.input}
                ...
                <View style={SignupStyles.signinContainer}>
                  <Text style={SignupStyles.signinText}>{t('alreadyHaveAccount')} </Text>
                  <TouchableOpacity onPress={navigateToSignin} activeOpacity={0.7}>
                    <Text style={SignupStyles.signinLink}>{t('signIn')}</Text>
                  </TouchableOpacity>
                </View>
              </View>
            </ScrollView>
